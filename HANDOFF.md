...
## 从 GitHub 在新电脑和新服务器首次实盘（2026-09-24）
- 用户明确表示旧 TEST 模拟成交无需恢复，并授权把**当前交易规则的具体参数**公开保存到 GitHub；不授权公开钱包秘密、服务器密码或交易历史。已新增 [docs/START_FRESH_LIVE.md](docs/START_FRESH_LIVE.md) 作为非技术用户入口，内有可直接发给未来 Codex 的说明。新服务器从空白账本开始前须先核对公开设置文件是否仍是用户要的版本；本程序后来一旦产生真实 LIVE 订单、持仓或待结算权利，就须迁移最新真实账本，不能再照搬“丢弃旧记录”的步骤。
- `config/strategy-profile.json` 仅含 `Preferences` 模型的 15 项设置。2026-09-24 通过服务器只读接口和 SQLite 核对：TEST 与 LIVE 的原始 `preferences` 完全相同，LIVE 原生事件和订单意图均为 0；公开文件的规范 JSON SHA-256 与旧服务器 TEST 原始设置同为 `17e9b3392c2da1e4b838bf2ed018292cbc280000f27a5875c234c06fa6c25653`。文件没有钱包地址、凭据、资金或交易记录。未来服务器改参数后该公开文件**不会自动同步**，须重新只读导出、核对并更新 GitHub。
- 新增只读导出工具 `scripts/export_preferences.py` 与仅对两个全新、未启动账本使用的 `scripts/apply_preferences.py`，配合新手指南在首次应用启动前导入 TEST 和 LIVE；导入工具拒绝用过的账本与超出默认 TEST 初始模拟资金的每轮金额，不改变用户当前 1U 规则。一个离线新账本的应用读回实测为 TEST/PAUSED、LIVE=false，两个模式 15 项设置均与公开文件在模型数值上相同。Mac 本地 134 项 Python 测试、Ruff 均通过。
- 首次 GitHub Actions 检查只在 Docker verify 阶段失败，因为新增测试需要的 `scripts/` 未被复制到测试镜像；基础运行镜像与原有应用代码未改变。修正 Dockerfile verify 阶段与 CI 对脚本的检查后，Actions run `35942184729` 的 Ubuntu x86_64/ARM64 作业均通过。服务器源代码在本次交付中同步至最新 main，运行中的应用仍使用 `9e30a87afd30a48fa0260972265364b872da1b1b` 的相同应用代码镜像；本次仅增加文档、未来安装工具、测试和 Docker verify 设置，无需为了它们打断 TEST。只读巡检实测仍为 TEST/RUNNING、LIVE=false、`backgroundErrors={}`、SQLite quick_check=ok。
- 尚未接入真实凭据、未解锁钱包、未启动 LIVE，也未进行真实签名、下单、批准或赎回。现有服务器位于硅谷，未来换服务器时应从实际交易出口按 Polymarket 官方地区接口核对是否允许 API 新开仓；只读私有预检不包含这项检查，也不枚举钱包所有手工持仓。用户仍需在可信终端掌握可导出的签名私钥；GitHub 不会保存或找回它。
